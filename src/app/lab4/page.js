...
<div className="p-10 bg-white  h-fit w-screen">
      <div className="flex flex-row w-full mx-auto mb-5 justify-between fixed">
        <input
            type="text"
            placeholder="Search users..."
            value={val}
            onChange={handleSearch}
            className="w-1/4 p-2 border border-gray-300 bg-gray-700 rounded-md shadow-sm focus:outline-none placeholder-black text-black focus:ring-2 focus:ring-blue-500"
          />
          <div className="w-1/4 flex flex-row">
          <div className="w-1/2  mr-32 bg-emerald-200 flex items-center justify-center rounded-2xl hover:cursor-pointer font-semibold" onClick={() => setTogle(!togle)}><p className="text-center ">Toggle</p></div>
          <div className=" w-1/2 mr-32 bg-blue-500 flex items-center justify-center rounded-2xl hover:cursor-pointer font-semibold " onClick={() => router.back()}><p className="text-center ">Back</p></div>
          </div>
      </div>
      {togle ? (<div className="grid lg:grid-cols-4 sm:grid-cols-1 gap-24 mt-20">
      {filtered.length >0 ? (
      filtered.map((element) => (
          <div
            key={element.id}
            className="w-84 h-84 flex flex-col p-5 rounded-lg text-black shadow-xl border-black bg-slate-200 transition delay-100 duration-200 ease-in-out hover:-translate-y-1 hover:scale-110 hover:bg-red-300"
          >
            <div
...
<img
            ...
            <p className="text-cyan-900 ml-2">Job: {element.job}</p>
